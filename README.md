...
あるいはこの正直な計算が一方向性を持つなら、この問題を使った新しい暗号も作れるのかもしれません。


追記：ここで符号理論の常識からはみ出してみましょう。
...
例えば、３２バイトのシンドロームに対応するエラーベクトルを計算したいものとする。
この場合、リードソロモン符号を使うなら、ハミング重み３２のエラーを計算するために、任意の３２個の位置から符号列を取り出す。
そうして作られた正方行列の逆行列を計算してやり、シンドロームにかけてやれば、エラーベクトルが決まるという訳です。
