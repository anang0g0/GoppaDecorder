...
例えばマイニングの作業はフォーマットにあったハッシュ値を探すという計算をスパコンでしますが、
符号で同じようなことをする場合、任意のシンドロームに対応するエラーと符号の対を探すという計算がマイニングに使えるのではと思っています。
これはマイニングという作業を利用してIDベース暗号を実現しようというものです。
今は確率的アルゴリズムなので計算量による力づくの計算が必要になりますが、決定性アルゴリズムができればPoWは必要なくなります。
今のところ、鍵が本人のものであることをPoWで保証するというような感じです。
また力づくの計算無しでPoWを実現する方法も考案中です。

ところがエンジニアさんの意見では、ビットコインのマネをしてはいけないという忠告を受けているですね。
まだその理由はよくわかりませんけど、そのエンジニアさんが何か別の方法を考えている可能性はあります。
