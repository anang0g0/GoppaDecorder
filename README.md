...
ちょっと考えたら別の方法があることに気が付きました。
まずパリティ検査行列から1/g(x)^2を取り除いて、Vandermonde行列に直します。  
（実際やってみたらうまく行きました！）  
こうすることで、検査行列を単純化して奇数次の列から偶数時の列を作れるかもしれない。
あまり期待してないですがやって見る価値はありそうです。
そしてGoppaにしたいときは、各列に対して1/g(x)^2をかければいいだけなので、変換は簡単にできるはずです。
