...
それと謎なのは、バーレカンプの方法ではシンドロームだけで訂正可能だということ。
これがうまく行くということは、どんな符号でも訂正できるということになり、Goppa符号を使う意味がなくなってしまう。
...
BM法を実装。
まだまだ小さい例しか試してないけど、どんなものかは理解できた。
4つの式がネストしているのでわかりにくいが、プログラミングの難易度としてはパターソンのほうが上だと思う。
もっと手間取るかと思った。
条件分岐が適当なので実用としてはまだ使えない。
4つの式が絡み合っている漸化式になっているので、すぐにはわからないけど気合で実装ｗ

あと問題なのが未知のシンドロームを計算できるようにすること。
Niederreiterの場合McElieceみたいに受信語があるわけじゃないから、単純にパリティ検査行列を2倍にすればいいという問題ではない。
