...
また、それとは別の方法として、もしこの行列が符号であれば、符号の列にランダムな列データを挿入してランダム化した行列を公開鍵にすることは、
符号の代数的構造を崩すための一つの方法として可能性がある。
なぜなら符号の計算は、隣り合った列から干渉を受けないため、正しいシンドロームの値は、
どの位置に符号の列があるか識別することが難しければ、符号の位置を知っている人でなければ復号できないからである。

もしかしたらこの方法でバイナリではなく、バイト誤Goppa正符号でも安全な公開鍵暗号ができるかもしれない。
