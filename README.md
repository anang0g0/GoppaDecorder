...
ここで符号理論の常識からはみ出してみましょう。
通常、エラーとシンドロームが１対１の関係であるためには、エラーベクトルの重みが決まっていなければなりません。
しかしもしこの条件を取り除いて、シンドロームと同じサイズのエラーを許せば、
部分符号の逆行列を計算するだけでどのような符号からも簡単にシンドロームに対応するエラーベクトルが決定できます。
しかしそのような場合でも、長い符号のどの位置にそのベクトルが来るのか計算できなければ組み合わせ論的な計算爆発が起きて安全であることが保証できます。
なのでより少ない重みのエラーベクトルでシンドロームと対応させる必要があるかもしれません。
