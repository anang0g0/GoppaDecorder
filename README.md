...
clang-11でもウゴクようになったけど、基本的なバグがあることが判明。
０の位置にエラーがあると、誤りロケータが正しくに計算されないなど。

他にもゆるいバグがあるので、体力と意志がある限り開発に専念したい。

仕様なのか、gccでコンパイルするのは死ぬほど遅いのでclangでコンパイルすることを強くおすすめする。
それとclangはプログラムの記述とかメモリにシビアなせいか、ちょっとでも使わない配列操作とか書いてるだけでも全然動作が変わってしまう。

これは仕様というより、clangのバグのような気もするけどどうなんだろう。
