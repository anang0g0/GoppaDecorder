...
ただUUIDの関係で起動するのに3分もかかる（SSDなのに）のでその解決法を探しています。
今度の新しいDeban１１が出るまで放置でもいいのですが、結局開発環境がLinuxしかないということなのでLinux以外で動かせないという事になります。
この辺なんとか改善したいです。
...
原因は復号に使うgcd関数だった。
昔書いたxgcdに戻したらうまく行った。
まだ０の位置にエラーがあると正しく動かないけど、こんなちょっとした動作の違いで計算結果が全く合わなくなる（そういうものだ）。
